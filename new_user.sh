...
read password
if [ -n "$password" ]; then
    password=$(python -c 'from hashlib import pbkdf2_hmac; from base64 import b64encode; print b64encode(pbkdf2_hmac("sha256", bytearray("'$password'"), bytearray("'$user_name'"), 30000))')
    break
fi
done
